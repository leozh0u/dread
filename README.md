...
Your webcam reads your pulse (via [Presage](https://presagetech.com)'s SmartSpectra SDK, with no
wearable and no wristband). The house watches your heart rate against your own calm baseline:

- **Calm → it comes closer.** Footsteps. A door left open. Something at the edge of the flashlight.
- **Terrified → it backs off.** Total silence. Nothing. It is waiting for
...
with ~20 data points in a single play session, a multi-armed bandit is the right tool.
...
edge of what they can tolerate, not past it. We think that's an interesting result of
...
we introduced, and any product built on this needs to account for it before it's
...
failsafe exists for exactly one reason: **live judging happens 3 to 4 times, cold, in a loud room, and
